...
TEST_F(FutureThenUnwrap, future_unwrapped) {
  ...
  auto cnt_f = future.then(
      [inner_future = inner_promise.get_future()](pc::future<int>) mutable { return std::move(inner_future); });
...
TEST_F(FutureThenUnwrap, shared_future_unwrapped) {
  pc::promise<void> inner_promise;
  auto cnt_f = future.then(
      [inner_future = inner_promise.get_future().share()](pc::future<int>) mutable { return std::move(inner_future); });
...
TEST_F(FutureThenUnwrap, result_is_ready_after_continuation_result_becomes_ready) {
  pc::promise<void> inner_promise;
  pc::future<void> cnt_f = future.then([&](pc::future<int>) { return inner_promise.get_future(); });
  promise.set_value(42);
...
TEST_F(FutureThenUnwrap, exception_from_unwrapped_continuation_propagated_to_result) {
  pc::future<std::unique_ptr<int>> cnt_f =
      future.then([](pc::future<int>) -> pc::future<std::unique_ptr<int>> { throw std::runtime_error("Ooups"); });
  promise.set_value(42);
  EXPECT_RUNTIME_ERROR(cnt_f, "Ooups");
}
...
TEST_F(FutureThenUnwrap, run_continuation_on_specific_executor) {
  pc::future<std::thread::id> cnt_f = future.then(
      g_future_tests_env, [](pc::future<int>) { return pc::make_ready_future(std::this_thread::get_id()); });
  promise.set_value(42);
  EXPECT_TRUE(g_future_tests_env->uses_thread(cnt_f.get()));
}

TEST_F(FutureThenUnwrap, shared_future_and_unwrapped_shared_future_access_same_storage) {
  pc::promise<std::string> inner_promise;
  pc::shared_future<std::string> inner_future = inner_promise.get_future();
  pc::shared_future<std::string> cnt_f = future.then([inner_future](pc::future<int>) { return inner_future; });
