...
TEST_F(FutureNext, continuation_executed_on_specified_executor) {
  pc::future<std::thread::id> cnt_f = future.next(g_future_tests_env, [](int) { return std::this_thread::get_id(); });
  promise.set_value(42);
...
TEST_F(FutureNext, upwraps_shared_future) {
  auto cnt_f = future.next([](int) { return pc::make_ready_future(100500).share(); });
...
TEST_F(FutureNext, desroys_continuation_after_invocation) {
  ...
  auto cnt_f = future.next([sp = std::exchange(sp, nullptr)](int val) { return val + *sp; });
