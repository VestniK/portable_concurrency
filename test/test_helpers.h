...
template <>
inline int some_value<int>() {
  return 42;
}

template <>
inline std::string some_value<std::string>() {
  return "hello";
}

template <>
inline std::unique_ptr<int> some_value<std::unique_ptr<int>>() {
  return std::make_unique<int>(42);
}

template <>
inline future_tests_env& some_value<future_tests_env&>() {
  return *g_future_tests_env;
}

template <>
inline void some_value<void>() {}
...
template <>
inline void set_promise_value<void>(pc::promise<void>& p) {
  p.set_value();
}
...
template <>
inline pc::future<future_tests_env&> make_some_ready_future() {
  return pc::make_ready_future(std::ref(some_value<future_tests_env&>()));
}

template <>
inline pc::future<void> make_some_ready_future() {
...
auto set_error_in_other_thread(std::chrono::duration<R, P> sleep_duration, E err) {
  pc::promise<T> promise;
  auto res = promise.get_future();

  g_future_tests_env->run_async(
      [](pc::promise<T>&& promise, E worker_err, std::chrono::duration<R, P> tm) {
...
inline std::string to_string(const std::unique_ptr<int>& val) { return val ? to_string(*val) : "nullptr"; }
...
inline void expect_some_value(pc::shared_future<void>& f) { EXPECT_NO_THROW(f.get()); }
