...
TEST_F(FutureThen, contunuation_receives_future) {
  auto cnt_future =
      future.then([](auto&& f) { static_assert(std::is_same<std::decay_t<decltype(f)>, pc::future<int>>::value, ""); });
}
...
TEST_F(FutureThen, exception_from_continuation_delivered_to_returned_future) {
  pc::future<bool> cnt_f = future.then([](pc::future<int>) -> bool { throw std::runtime_error("continuation error"); });
  promise.set_value(42);
  EXPECT_RUNTIME_ERROR(cnt_f, "continuation error");
}

TEST_F(FutureThen, value_is_delivered_to_continuation_for_not_ready_source) {
  pc::future<void> cnt_f = future.then([](pc::future<int> f) { EXPECT_EQ(f.get(), 42); });
  promise.set_value(42);
  cnt_f.get();
}
...
TEST_F(FutureThen, exception_to_continuation) {
  ...
  pc::future<std::string> string_f = f.then([](pc::future<int> ready_f) -> std::string {
...
TEST_F(FutureThen, run_continuation_on_specific_executor) {
  pc::future<std::thread::id> cnt_f =
      future.then(g_future_tests_env, [](pc::future<int>) { return std::this_thread::get_id(); });
  set_promise_value(promise);
