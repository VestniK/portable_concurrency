...
void get_task_future_twice() {
  ...
  EXPECT_FUTURE_ERROR(f2 = task.get_future(), std::future_errc::future_already_retrieved);
}
...
void failed_call_makes_state_ready() {
  pc::packaged_task<T()> task([]() -> T { throw std::runtime_error("operation failed"); });
  pc::future<T> f = task.get_future();
  ASSERT_TRUE(f.valid());
  EXPECT_FALSE(f.is_ready());
...
void copyable_functor_task() {
  ...
  static_assert(!std::is_function<decltype(func)>::value && std::is_copy_constructible<decltype(func)>::value,
